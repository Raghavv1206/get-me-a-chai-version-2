...
export const authOptions = {
    ...
    callbacks: {
                ...
                try {
                    ...
                    if (!existingUser) {
                        ...
                        await User.create({
                            ...
                            username: username,
                            profilepic: user.image || "/images/default-profilepic.svg"
                        });
                    }
                } catch (error) {
                    // Log the error but still allow sign-in to proceed if user was already created
                    console.error('Error in signIn callback:', error.message);
                    // If it's an auth failure, block sign-in so the user gets a clear error
        ...
        async jwt({ token, user, account }) {
            ...
            // Fetch latest user data from DB and sync to token
            // Wrapped in try/catch so a temporary DB outage doesn't kill the session
            if (token.email) {
                try {
                    await connectDb();
                    ...
                    // Gracefully degrade: keep the existing token data so the session
                    // remains valid even if MongoDB is temporarily unavailable.
                    // This prevents cascading 401s/session failures on DB hiccups.
                    console.error('Error fetching user in JWT callback:', error.message);
                }
            }
