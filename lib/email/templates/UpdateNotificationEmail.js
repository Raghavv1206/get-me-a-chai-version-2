...
import { buildCampaignUrl, getBaseUrl } from '@/lib/urlHelpers';
 ...
 * @param {string} data.campaignId - Campaign MongoDB ObjectId
...
export function UpdateNotificationEmail({
  ...
  const updateUrl = buildCampaignUrl(campaignId, { hash: `update-${updateId}`, absolute: true });
  const campaignUrl = buildCampaignUrl(campaignId, { absolute: true });
