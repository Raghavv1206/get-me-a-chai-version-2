...
const config = {
    ...
    // NOTE: No global Razorpay env vars are used for payment processing.
    // Each creator stores their own Razorpay Key ID & Secret in their profile (DB)
    // via the Settings page. Per-user credentials are fetched at payment time.
    payment: {
        razorpay: {
            currency: getEnv('PAYMENT_CURRENCY', 'INR'),
            minAmount: getIntEnv('PAYMENT_MIN_AMOUNT', 1),   // ₹1
            maxAmount: getIntEnv('PAYMENT_MAX_AMOUNT', 9999999), // ₹99,99,999
        },
    },
...
function validateConfig() {
    ...
    // NOTE: Razorpay Key ID/Secret are no longer required env vars.
    // Each creator provides their own credentials via the Settings page (stored in DB).
...
// Validate on module load (server-side only, skip during Next.js build phase)
...
if (typeof window === 'undefined' && !isBuildPhase) {
    try {
        ...
        if (config.isProduction && !isBuildPhase) {
            logger.error('Missing required environment variables. Please check your Vercel/hosting environment settings.');
            // Don't call process.exit(1) here as it kills the build worker
        }
    }
}
